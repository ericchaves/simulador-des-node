...
describe('LinhaDoTempo', () => {
  ...
  test('deve agendar um novo evento para ser disparado quando o horário chegar', () => {
    const evento: Evento = { emissor: mockEntidade, nome: 'eventoTeste', entidade: 'entidadeTeste', espera: 5, argumentos: new Map<string, any>() };
  ...
  test('deve avançar o tempo ao emitir os eventos agendados para o futuro', () => {
    linhaDoTempo.agendar({ emissor: mockEntidade, nome: 'eventoTeste', entidade: 'entidadeTeste', espera: 1, argumentos: new Map<string, any>() });
    const gerador = linhaDoTempo.avancarTempo();
  ...
  test('ao emitir eventos agendados com espera zero não deve avançar o tempo', () => {
    linhaDoTempo.agendar({ emissor: mockEntidade, nome: 'eventoTeste', entidade: 'entidadeTeste', espera: 0, argumentos: new Map<string, any>() });
    const gerador = linhaDoTempo.avancarTempo();
    const resultado = gerador.next();
    expect(resultado.done).toBe(false);
    expect(resultado.value.nome).toBe('eventoTeste');
  ...
  test('deve emitir eventos na ordem correta quando múltiplos eventos são agendados para o mesmo momento', () => {
    linhaDoTempo.agendar({ emissor: mockEntidade, nome: 'evento1', entidade: 'entidadeTeste', espera: 0, argumentos: new Map<string, any>() });
    linhaDoTempo.agendar({ emissor: mockEntidade, nome: 'evento2', entidade: 'entidadeTeste', espera: 0, argumentos: new Map<string, any>() });
    const gerador = linhaDoTempo.avancarTempo();
  ...
  test('deve emitir eventos na ordem correta quando múltiplos eventos são agendados fora de ordem momento', () => {
    linhaDoTempo.agendar({ emissor: mockEntidade, nome: 'evento2', entidade: 'entidadeTeste', espera: 1, argumentos: new Map<string, any>() });
    linhaDoTempo.agendar({ emissor: mockEntidade, nome: 'evento1', entidade: 'entidadeTeste', espera: 0, argumentos: new Map<string, any>() });
    linhaDoTempo.agendar({ emissor: mockEntidade, nome: 'evento3', entidade: 'entidadeTeste', espera: 1, argumentos: new Map<string, any>() });
    const gerador = linhaDoTempo.avancarTempo();
    expect(gerador.next().value.nome).toBe('evento1');
    expect(gerador.next().value.nome).toBe('evento2');
